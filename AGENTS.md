...
4. Schedules cross-platform via **Zernio** (Instagram / TikTok / YouTube / X).
...
- Don't read or print secrets from `.env*`, Supabase service keys, Stripe keys, Luma keys, or Zernio tokens unless the user explicitly asks for a security audit.
- Never commit `.env*` files (only `.env.example`).
- Server-only modules (`src/lib/{stripe,zernio,luma}.ts`, `src/lib/supabase/server.ts`, `src/lib/env.ts`'s `serverEnv`) must never be imported from a client component.
- Treat Luma + Zernio as paid APIs: keep usage explicit, log requests, never burn credits in tests.
...
- For Luma/Zernio/Stripe integrations: mock the SDK at the boundary in tests; only hit the real API behind an explicit env flag.
